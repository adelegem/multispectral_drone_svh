...
- `figures-for-publication.R` — **manuscript PNG exporter**. Retained alongside the inline-figure `report.html` because it's the only thing producing the 600-dpi PNGs at the sizes the manuscript submission needs. Reads from `data_out/` (the legacy script outputs), not from the targets cache — so running it requires running `continuous_metrics_analysis.R` first. Migrating it to `tar_read()` is the obvious next cleanup but isn't blocking publication; current users producing manuscript figures should keep using the legacy chain.
- `tools/` — release-time / maintenance helpers; not invoked by the pipeline.
  - `tools/checksums.R` (Phase 5.6): re-prints the canonical pipeline-output digests for verifying a fresh reproduction.
  - `tools/pack_cache.R` (Phase 5.7 prep): packages a lightweight `_targets_lite.tar.gz` (~2 MB, every cached object except the bulky `pixel_values_*` matrices) for attaching to the GitHub release. Refuses to package an outdated cache. Locally verified to deliver ~10–15 min reproduction from a fresh checkout — see README's *Faster partial reproduction*.
- `preprocessing/` — **upstream of `tar_make()`, not part of the targets pipeline.** Scripts that produced the masked rasters archived on Zenodo (Pix4D ortho mosaics → band stacking → ROC-thresholded NIR+NDVI masking). Run once per dataset; outputs are persistent. Has its own [README](preprocessing/README.md) describing inputs/outputs and the three-stage script breakdown (band stacking → ROC threshold selection → mask application; only stage 2's `02_roc_thresholds.R` is reproducible in-repo, the other two need the raw Pix4D outputs). Source `funx.R` for the preprocessing-only helpers (`create_masked_raster`, `find_optimum_thresholds`).
...
- `reports/` — tracked: `report.Rmd` + its rendered `report.html` (Phase 5.3, rebuilt by `tar_render()` whenever upstream targets change). The directory itself is **not** in `.gitignore` (only `*.log` is); the transient figure drafts (`workflow_*.png`, `workflow_*.mmd`) ARE gitignored. The pre-Phase-4 `interim_progress.Rmd` + `main_results_summary.txt` were retired in Phase 5.8 in favour of `report.html`.
...
For current results (figures, model summary tables, per-target runtime), see `reports/report.html` — rendered by `tar_render()` and rebuilt automatically whenever an upstream target changes. For the live DAG of which targets are up to date, `targets::tar_visnetwork()` after `tar_make()`. CLAUDE.md only documents durable structure, conventions, and the phase plan; transient analysis state lives in the report.
...
- **`tests/test-pack-cache.R` (auto-skipping).** Exercises `tools/pack_cache.R` end-to-end: requires a clean (`tar_outdated()` empty) `_targets/` cache, runs the packaging script, asserts the tarball includes `meta/meta`, excludes every `pixel_values_*` object, has no missing non-pixel objects, and stays under 50 MB. Skips on a fresh checkout (no cache) and in CI — so it adds nothing to Tier 1's runtime, but acts as a built-in regression check the next time a maintainer runs the test suite locally after `tar_make()`.
...
| 5.6 | Output checksums as release artifacts | ✓ done |
| 5.7 | Code DOI via Zenodo | in progress — integration enabled; concept DOI `10.5281/zenodo.20387946` minted (from `v0.0.1` test release). Remaining: cut `v1.0.0`. |
| 5.8 | Repo housekeeping pass | ✓ done |
...
6. **Output checksums as release artifacts.** ✓ done. `tools/checksums.R` regenerates the MD5 digests of the six canonical pipeline outputs plus the three tracked test fixtures. Current digests live inline in the README's *Verifying your reproduction* section so readers comparing against their own `tar_make()` don't have to wait for a release. At v1.0.0 cut: re-run `tools/checksums.R` against a fresh full `tar_make()`, paste any changed digests into the README + release notes, and (optionally) attach the actual RDS files as release assets (`gh release upload v1.0.0 <files>`).
7. **Code DOI via Zenodo–GitHub integration.** Integration enabled; the `v0.0.1` test release already minted a code DOI (concept DOI `10.5281/zenodo.20387946`, version DOI `10.5281/zenodo.20387947`), separate from the data DOI (10.5281/zenodo.17089161). The concept DOI is version-independent and resolves to the latest release, so it's wired into the README badges + Citation section and `CITATION.cff` (`identifiers:` block). Remaining: cut `v1.0.0` so the concept DOI resolves to a complete documented release, and cite both DOIs in the manuscript.
8. **Repo housekeeping pass.** ✓ done. Drift fixes had already been folded into the relevant doc-update commits as they were spotted (e.g. `reports/` gitignored claim, workflow NDVI/NIR/Red → NIR+NDVI only, Phase-4-as-plan-vs-built reframing, the data_out/ legacy-output relabel). Final dedicated sweep before the v1.0.0 release: (a) retired the pre-Phase-4 `reports/interim_progress.Rmd` + `.html` and `main_results_summary.txt` since `report.html` is now canonical; (b) deleted the transient workflow draft files and added `reports/workflow_*.png|.mmd` to `.gitignore`; (c) updated `figures-for-publication.R`'s docstring to acknowledge its "manuscript PNG exporter" role; (d) cleaned up stale "Phase 4b will populate" comment in `.gitignore`; (e) grep'd the README + CLAUDE.md + `_targets.R` + `funx.R` for residual references to retired files and updated each one. The `figures-for-publication.R` → `tar_read()` migration is the obvious next cleanup but not blocking publication; recorded as a follow-up.
