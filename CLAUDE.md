...
- `reports/` — tracked: `report.Rmd` + its rendered `report.html` (Phase 5.3, rebuilt by `tar_render()` whenever upstream targets change), plus the legacy `interim_progress.Rmd`/`interim_progress.html` (pre-Phase 4) and `main_results_summary.txt`. The directory itself is **not** in `.gitignore` (only `*.log` is); transient figure drafts (`workflow_*.png`, `workflow_*.mmd`) are intentionally untracked. Phase 5.8 will decide the fate of the interim report now that `report.Rmd` is canonical.
